...
1. Switch back to the Azure Potral and the log analytics workspace
...
10. Type **IP_CF** for the field title
11. For the field type, select **Text**
12. Select **Extract**
13. Select **Save extraction**

14. Run the following query to show data based on a specific IP address:
...
2. Run the following query in your log analytics workspace to see the data you just imported:

    ```output
    OrgSecurity_CL
    ```

3. Run the following query to show data based on a specific IP address:

    ```sql
    OrdSecurity_CL
    | where IPAddress == "192.168.102.2"
    ```
