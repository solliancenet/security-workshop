...
7. Copy the device code and click the **Copy code to dashboard and authenticate**

8. In the dialog that opens, copy the device code

9. Login using the lab username and password
...
13. The next cell will execute a query passing in parameters:

    ![Cell output is displayed.](./media/sentinel-notebook-cell-log-analytics-query-execution.png "Review the output")

14. Run the remaining cells, you will make calls to the external API providers to enrich the log data with threat levels and IP Address analysis
