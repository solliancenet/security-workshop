...
5. Open the **Putty** tool, login to the **wssecuritySUFFIX-linux-1** machine using the `wsuser` username and lab password.  The IP Address should be `10.0.0.4`.
...
2. Open the `C:\LabFiles\Artifacts\BruteForce.ps1` in a PowerShell ISE window
...
1. Switch to the Remote Desktop Connection to the **wssecuritySUFFIX-paw-1**.
2. Open **Event Viewer**.
...
4. If prompted, select **Try Application Whitelisting**
   - If prompted, select your workspace, then click **Upgrade**
   - Select **Continue without installing agents**
5. You will likely have several groups displayed, find the one that has your newly created lab VMs.

   ![Machine groupings is displayed.](media/securitycenter-grouping.png "Azure automatically created a group for your VMs")

6. Expand the **Publisher whitelisting rules** section, you should see that Google Chrome and Notepad++ were picked up and have Microsoft Certificated tied to them
...
7. In the top menu, select **Group settings**.
...
7. If displayed, select **Enable**, then select **Enable File Integrity Monitoring**. If not displayed, simply select the workspace.
...
17. Select **Link**, then select the storage account tied to your lab.
...
18. Select **Save**
19. Switch to the Remote Desktop Connection to the **paw-1**.
20. Open the **c:\windows\system32\drivers\etc\hosts** file.
...
22. Save the file.
23. After about 30-60 minutes, the Log Analytics workspace will start to pickup changes to your files, registry settings and windows services:
...
24. You will also start to see the file snapshots show up in the storage account:
...
5. Select the **Azure Disk Encryption for volume encryption** checkbox.
   ...
   ![The click path above is highlighted.](media/fileintegrity-snapshots.png "Enable the key vault for disk encryption activities")
...
10. In the top menu, select **Encryption**.
    ...
    ![The click path above is highlighted.](media/fileintegrity-snapshots.png "Browse to configure disk encryption for the linux-1 os disk")
