...
3. On the getting started page, select **Upgrade**
...
2. Open the `C:\LabFiles\security-workshop\Artifacts\BruteForce.ps1` in a PowerShell ISE window

3. Run the script, notice how it will execute several attempts to login via SSH to the **wssecuritySUFFIX-linux-1** machine using the plink tool from putty.
...
7. Select the log analytics workspace for your resource group and then select the matching automation account, then select **Ok**.
    ...
    ![The custom configuration dialog is displayed with the log analytics workspace select along with the matching automation account.](media/virtual-machines-svcs-changetracking-config2.png "Select the resource group log analytics workspace and matching automation account")

> **NOTE** See [Region Mappings](https://docs.microsoft.com/en-us/azure/automation/how-to/region-mappings) for more information.

1. Select all the virtual machines, then select **Enable**.
2. Navigate back to the **Virtual Machines** blade, again highlight the **paw-1** and **linux-1** virtual machines that were deployed.
3. In the top menu, select **Services**, then select **Inventory**.
4. Select the **CUSTOM** radio button.
5. Select **change**, select the **Log Analytics Workspace** that was deployed with the lab ARM template.
6. Notice that all the VMs are already enabled for the workspace based on the last task.
7. Navigate back to the **Virtual Machines** blade, again, highlight the **paw-1** and **linux-1** virtual machines that were deployed.
8. In the top menu, select **Services**, then select **Update Management**.
9. Select the **CUSTOM** radio button.
10. Select **change**, select the **Log Analytics Workspace** that was deployed with the lab ARM template.
11. Select all the virtual machines, then select **Enable**.
12. Browse to your resource group, then select your Log Analytics workspace.
13. Under the **General** section, select the **Solutions** blade, you should see the **ChangeTracking** and **Updates** solutions were added to your workspace. Select the **ChangeTracking** solution.
...
14. Under **Workspace Data Sources** section, select **Solution Targeting (Preview)**.
15. Remove any scopes that are displayed via the ellipses to the right of the items.
16. Repeat the steps to remove the solution targeting for the **Updates** solution.
...
5. For the log workspace tied to your lab environment virtual machines, is displayed, select **Upgrade Plan**, then select **Try File Integrity Monitoring**.
...
9. In the menu, select **Settings**.

    ![The Settings link is highlighted.](media/fileintegrity-settings.png "Select the settings link")

10. Select the **Windows Files** tab.
11. Select **+Add**.
12. For the item name, type **HOSTS**.
13. For the path, type **c:\windows\system32\drivers\etc\\\***.
14. Toggle **Recursion** to **On**
15. Toggle **Upload file content** to **True**
16. Select **Save**.
...
17. Select the **File Content** tab.
18. Select **Link**, then select the storage account tied to your lab.
...
19. Select **Save**
20. Switch to the Remote Desktop Connection to the **paw-1**.
21. Open the **c:\windows\system32\drivers\etc\hosts** file.
...
23. Save the file.
24. After about 30-60 minutes, the Log Analytics workspace will start to pickup changes to your files, registry settings and windows services:
...
25. You will also start to see the file snapshots show up in the storage account:
