...
<#
    $logs = Get-EventLog -LogName Microsoft-Windows-TerminalServices-RemoteConnectionManager/Operational;
    Get-WinEvent -LogName "Microsoft-Windows-TerminalServices-RemoteConnectionManager/Operational"
#>

function SetupRegistryAccess($key)
{
    $key = 'HKCU:\AppEvents\EventLabels\ActivatingDocument'
...
function ElevateUserAndProcess()
{
    foreach($name in [PS_LSA.Rights]::GetNames([PS_LSA.Rights]))
    {
        ...
        try
        {
            Grant-UserRight -Account "wsuser" -Right $name;
        }
        catch
        {
            write-host $name;
    ...
    foreach($name in [PS_LSA.Rights]::GetNames([PS_LSA.Rights]))
    {
        ...
        Grant-TokenPrivilege -Right $name;
    }

    & C:\labfiles\sepriv64.exe SeAuditPrivilege
    & C:\labfiles\sepriv64.exe SeChangeNotifyPrivilege
    & C:\labfiles\sepriv64.exe SeImpersonatePrivilege

}

function CreateSuccessfulWinLogin()
{
    $ht = new-object System.Collections.Hashtable;
    $ht.add("LogName","Security")
    $ht.add("ID","4625")

    #TODO
}

function CreateFailureWinLogin()
{
    ElevateUserAndProcess

    $logs = get-winevent -FilterHashtable @{ LogName='Security'}
    $logs = $logs[0];

    $subjectUsersSid = "S-1-5-18";
    $SubjectUserName = "SYSTEM";
    $SubjectDomainName = 'NT AUTHORITY'
    $SubjectLogonId = 0x3e7;
    $TargetUserSid = $Null #inType="win:SID" outType="xs:string"/>
    $TargetUserName = $null #" inType="win:UnicodeString" outType="xs:string"/>
    $TargetDomainName = $null # inType="win:UnicodeString" outType="xs:string"/>
    $Status = $null # inType="win:HexInt32" outType="win:HexInt32"/>
    $FailureReason = $null #" inType="win:UnicodeString" outType="xs:string"/>
    $SubStatus = $null #" inType="win:HexInt32" outType="win:HexInt32"/>
    $LogonType = $null #" inType="win:UInt32" outType="xs:unsignedInt"/>
    $LogonProcessName = $null #" inType="win:UnicodeString" outType="xs:string"/>
    $AuthenticationPackageName = $null #" inType="win:UnicodeString" outType="xs:string"/>
    $WorkstationName = $null #" inType="win:UnicodeString" outType="xs:string"/>
    $TransmittedServices = $null #" inType="win:UnicodeString" outType="xs:string"/>
    $LmPackageName = $null #" inType="win:UnicodeString" outType="xs:string"/>
    $KeyLength = $null #" inType="win:UInt32" outType="xs:unsignedInt"/>
    $ProcessId = $null #" inType="win:Pointer" outType="win:HexInt64"/>
    $ProcessName = $null #" inType="win:UnicodeString" outType="xs:string"/>
    $IpAddress = $null #" inType="win:UnicodeString" outType="xs:string"/>
    $IpPort = $null #" inType="win:UnicodeString" outType="xs:string"/>
    ...
    New-WinEvent -ProviderName Microsoft-Windows-Security-Auditing -Id 4625 -Payload $vals;
    
    Write-EventLog -LogName Security -Source Microsoft-Windows-Security-Auditing -Id 4625 -Message "Hi";
    
}

function CreateSuccessfulLogin($ip)
{
    New-WinEvent -ProviderName Microsoft-Windows-TerminalServices-RemoteConnectionManager -Id 1149 -Payload @("wsuser","MicrosoftAccount","$ip")
}

function CreateFailedLogin($ip)
{
    New-WinEvent -ProviderName Microsoft-Windows-RemoteDesktopServices-RdpCoreTS -Id 140 -Payload @("$ip")
    New-WinEvent -ProviderName Microsoft-Windows-RemoteDesktopServices-RdpCoreTS -Id 131 -Payload @("TCP","$ip")
}

function DoRDPAttack()
{
    #https://docs.microsoft.com/en-us/azure/sentinel/connect-windows-security-events
    
    $ips = @("203.160.71.100","117.82.191.160", "80.89.137.214");

    foreach($ip in $ips)
    {
        for($i=0;$i-lt100;$i++)
        {
            CreateFailedLogin $ip;
        }
    
        CreateSuccessfulLogin $ip;
    }
}

function DoFileModificationAttack
{
    #Modify a system file: 
}

function InstallAService
{
    #Generate a 4697(S): A service was installed in the system.
...
function GenerateSecurityEvents()
{
    ...
    #wevtutil set-log "Security" /ca:O:BAG:SYD:(A;;0x2;;;S-1-15-2-1)(A;;0x2;;;S-1-15-3-1024-3153509613-960666767-3724611135-2725662640-12138253-543910227-1950414635-4190290187)(A;;0xf0007;;;SY)(A;;0x7;;;BA)(A;;0x7;;;SO)(A;;0x3;;;IU)(A;;0x3;;;SU)(A;;0x3;;;S-1-5-3)(A;;0x3;;;S-1-5-33)(A;;0x1;;;S-1-5-32-573)
}
